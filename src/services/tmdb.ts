...
export const discoverMovies = async (genreIds: number[]): Promise<Movie[]> => {
  ...
  const data = await fetchFromTMDB(`/discover/movie?with_genres=${genres}&sort_by=popularity.desc`);
  return data.results;
};
...
export const getMovieDetails = async (id: number): Promise<Movie> => {
  ...
  const watchLink = usProviders.link || `https://www.justwatch.com/us/search?q=${encodeURIComponent(details.title)}`;
